...
library(psych)
library(GGally)
setwd("/data/WMjs/devAnalysis/v3")
...
oSpanPath <- paste0(
    dataDir,
    dataFiles[grep("operation-span", dataFiles)]
)
oSpanData <- read_csv(oSpanPath) |>
    filter(TestTrial, Source == "prolific") |>
    select(
        SbjID, Study, Source, OrigTime, StartTime, rt, stimulus,
        response, TrialN, Load, Operations, Letters, Score
    )
oSpanSummary <- oSpanData |>
    group_by(SbjID) |>
    summarise(
        oSpanPerf = mean(Score),
        oSpanNTrial = n()
    )
oSpanMatrix <- long2Matrix(oSpanData, "SbjID", "Score", "TrialN")
tmp <- oSpanMatrix[, seq_len(ncol(oSpanMatrix) - 1)]
tmp <- tmp[, !is.na(tmp[c("cor"), ])]
tmp <- tmp[seq_len(nrow(tmp) - 3), ]
oSpanRel <- splitHalf(tmp, check.keys = FALSE)$lambda2

allSummary <- inner_join(nBackSummary, bindingSummary, by = "SbjID") |>
    inner_join(oSpanSummary, by = "SbjID")

ggpairs(allSummary[c("nBackPerf", "BindingPerf", "oSpanPerf")])
